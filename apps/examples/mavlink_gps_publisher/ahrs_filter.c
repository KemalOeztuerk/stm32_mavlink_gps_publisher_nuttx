 ...
 * frame), which matches the DroneCAN RawIMU convention. */
                        ...
                        float gyro_x_rads, float gyro_y_rads, float gyro_z_rads)
{
     ...
     * filter -- the Here4's compass drives nav_state's heading separately
     * via dronecan_gnss.c), so it will drift over time. */
